...
namespace Ultimate_Carry_Prevolution.Plugin
{
	class Fiora : Champion
	{
		...
		private void Cast_R_MinHit()
		{
			...
			var enemies = xSLxOrbwalker.AllEnemys.Where(hero => hero.IsValidTarget(R.Range) && CountEnemyInPositionRange(hero.ServerPosition, 300) >= minEnemyHitCount).OrderBy(hero => hero.Health);
			if(enemies.Any())
			{
                Game.PrintChat("RHits: " + enemies.Count());
				var enemy = enemies.First();
				var enemyKillable = xSLxOrbwalker.AllEnemys.Where(x => x.ServerPosition.Distance(enemy.ServerPosition) <= 300 && R.GetDamage(x) > x.Health);
                Game.PrintChat("RKill: " + enemyKillable.Count());
